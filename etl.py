...
def process_song_file(cur, filepath):
    ...
    parameters:
        cur(cursor psycopg2): cursor to execute queries 
    ...
    song_data = [float(elem) if type(elem) is not str else elem for elem in song_data]
...
def process_log_file(cur, filepath):
    ...
    # load user table
    user_df = df.loc[:,['userId','firstName', 'lastName', 'gender', 'level']].drop_duplicates(subset=['userId'])
...
def process_data(cur, conn, filepath, func):
    """
    Access all files from defined path and process the files according to the func.
    
    parameters:
        cur(psycopg2 cursor): cursor to execute queries.
